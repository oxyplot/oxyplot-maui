...
namespace ExampleLibrary
{
    ...
    public static class Examples
    {
        ...
        {
            var orderedDic = new Dictionary<Type, int>();
            foreach (var type in typeof(Examples).GetTypeInfo().Assembly.DefinedTypes)
            {
                var ea = type.GetCustomAttributes<ExamplesAttribute>().FirstOrDefault();
                if (ea == null)
                {
                    continue;
                }
                orderedDic.Add(type, ea.DisplayOrder);
            }

            foreach (var kvp in orderedDic.OrderBy(x => x.Value))
            {
                foreach (var example in GetCategory(kvp.Key))
                {
