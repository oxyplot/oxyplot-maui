...
namespace ExampleLibrary
{
    ...
    public class ShowCases
    {
        ...
        [Example("2003年各月平均温度")]
        public static PlotModel LineLegendPositionAtEndUnicode()
        {
            // http://www.perceptualedge.com/example2.php
            var model = new PlotModel { Title = "2003年各月平均温度", PlotMargins = new OxyThickness(60, 4, 60, 40), PlotAreaBorderThickness = new OxyThickness(0), IsLegendVisible = false };

            const string TrackerFormatString = "{0}: {4:0.0}ºF";
            var phoenix = new LineSeries { Title = "凤凰城", LineLegendPosition = LineLegendPosition.End, TrackerFormatString = TrackerFormatString };
            var raleigh = new LineSeries { Title = "罗利", LineLegendPosition = LineLegendPosition.End, TrackerFormatString = TrackerFormatString };
            var minneapolis = new LineSeries { Title = "明尼阿波利斯", LineLegendPosition = LineLegendPosition.End, TrackerFormatString = TrackerFormatString };
            ...
            categoryAxis.Labels.AddRange(new[] { "一月", "二月", "三月", "四月", "五月", "六月", "七月", "八月", "九月", "十月", "十一月", "十二月" });
            model.Axes.Add(categoryAxis);
            model.Axes.Add(new LinearAxis { Position = AxisPosition.Left, Title = "华氏度", AxislineStyle = LineStyle.Solid });
